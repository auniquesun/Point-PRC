...
class ResidualAttentionBlock_IVLP(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, add_prompt=False,
        ...
        if i != 0:
            ...
            if self.add_prompt:
                if self.text_layer: # 区分 textual, visual prompts 的标记
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.add_prompt:
            ...
            if not self.text_layer:
                # --- prefix: (seq_len-n_ctx, batch, d_model) ---
                # NOTE Discard the outputs produced by learnable tokens of previous layers
                prefix = x[0:x.shape[0] - self.n_ctx_visual, :, :]

                # --- VPT_shallow.expand(x.shape[1], -1, -1): (batch, n_ctx, d_model) ---
                # --- VPT_shallow.expand(x.shape[1], -1, -1).permute(1, 0, 2): (n_ctx, batch, d_model) ---
                # --- visual_context: (n_ctx, batch, d_model) ---
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            # add learnable prompts in first `text_depth` layers => True; otherwise, => Fasle
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int,
        ...
        if self.VPT_shallow:
            ...
            # self.VPT.half()
        # --- 这里相当于 standard attention 中 softmax() 中的归一化元素 ---
        # scale: 768 ** -0.5
        scale = width ** -0.5
        ...
        self.prompt_till_layer_visual = design_details["vision_depth"]  # 9
        # --- NOTE 这里值得学习，vision transformer 中核心的模型，适合 text transformer 核心模型共享的 ---
    ...
    def forward(self, x: torch.Tensor):
        ...
        # --- class_embedding: (width,), torch.zeros: (batch, 1, width) ---
        # --- x: (batch, 1 + grid**2, width) ---
        x = torch.cat(
        ...
        if self.VPT_shallow:
            # --- self.VPT.expand(x.shape[0], -1, -1): (batch, n_ctx, width) ---
            visual_ctx = self.VPT.expand(x.shape[0], -1, -1).half()
            # x: (batch, 1 + grid**2 + n_ctx, width)
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        # hyper-parameter if need to add prompt embeddings inside to the input
        # of transformer block or not:
        prompt_till_layer_text = design_details['language_depth']   # 9
...
def build_model(state_dict: dict, design_details):
    ''' NOTE Create deep visual & textual prompts across multiple visual & textual transformer layers '''
    
    # vit-b/16 by default
    vit = "visual.proj" in state_dict

    if vit:
        # vision_width: 768, feature dimension of a visual token
        vision_width = state_dict["visual.conv1.weight"].shape[0]
        # vision_layers: 12, how many layers in visual transformer
        vision_layers = len(
        ...
        # image_resolution: 224,        width==height by default
    ...
    # transformer_width: 512    feature dimension
    transformer_width = state_dict["ln_final.weight"].shape[0]
    # transformer_heads: 8
    transformer_heads = transformer_width // 64
    # transformer_layers: 12    transformer layers in tex encoder
