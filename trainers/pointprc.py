...
class CustomULIP(nn.Module):
    def __init__(self, cfg, classnames, ulip_model):
        super().__init__()
        # NOTE the learnable text prompts is in `VLPromptLearner`
    ...
    def forward(self, pts, label=None):
        ...
        # NOTE `.training` is an attribute of nn.Module
...
class PointPRC(TrainerX):
    ...
    def forward_backward(self, batch):
        # NOTE this function is called once per `batch` in `TrainerX -> run_epoch()`
        ...
        if not self.cfg.TRAINER.PointPRC.NO_MEC:
            if (self.batch_idx + 1) == self.num_batches:
                self.update_lr()
                # Means one epoch is completed, perform MEC - gaussian weighted prompt aggregation
                # NOTE `self.step_counter` is set to 1 initially
    ...
    def state_dict_weighting(self, main_dict, weightage, prompt_only=False):
        ...
        if not prompt_only:
            # NOTE weigh all parameters with gaussian weights
    ...
    def state_dict_add(self, dict1, dict2, prompt_only=False):
        ...
        if not prompt_only:
            modified_dict = dict2
            for key in dict1:
                # NOTE update the whole weight space, not only learnable prompts
                #   Reference: Robust fine-tuning of zero-shot models, cvpr'22 --> zero-shot weights + finetuned weights
    ...
    def get_gauss(self, mu, sigma):
        ...
        #   the equation after `lambda: x` is a gaussian function
