    ...
    def frozen_text_emb(self, cfg, classnames, split_clsnames):
        ...
        if prompt_type == "manual64":
            ...
            for single_template in TEMPLATE:
                # `x` is list of length `num_classes`
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            if self.cfg.TRAINER.PointPRC.NO_MAC:
                ...
                # NOTE target is used in `F.cosine_embed_loss`
                tgt_txt = torch.ones(frozen_txt_feats.shape[0])
                tgt_point = torch.ones(frozen_point_feats.shape[0])

                # Calculate the L_MAC_text loss: use `mse` instead of `l1`
                loss_mac_text = F.cosine_embedding_loss(prompt_txt_feats, frozen_txt_feats.cuda(), tgt_txt.cuda(), 
                                    reduction='mean') * self.cfg.TRAINER.PointPRC.TEXT_LOSS_WEIGHT
                # Calculate the L_MAC_point loss: use `mse` instead of `l1`
                loss_mac_point = F.cosine_embedding_loss(prompt_point_feats, frozen_point_feats.cuda(), tgt_point.cuda(), 
        ...
        if not self.cfg.TRAINER.PointPRC.NO_MEC:
            if (self.batch_idx + 1) == self.num_batches:
                # NOTE update lr per epoch, instead of per step
                self.update_lr()
