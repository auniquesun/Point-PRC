...
class PointPRC(TrainerX):
    ...
    def build_model(self):
        ...
        uniform = self.get_uniform(N)     # `lambda` expression
        self.uniform = np.array([uniform(N) for _ in range(1, N + 1)])  # self.uniform: np.array([1/N, 1/N, ..., 1/N])
    ...
    def forward_backward(self, batch):
        ...
        if not self.cfg.TRAINER.PointPRC.NO_MEC:
            if (self.batch_idx + 1) == self.num_batches:
                ...
                current_epoch_weight = self.uniform[self.step_counter - 2]
            ...
            if self.step_counter == self.model.total_epochs + 1:
                print("Using Uniform Prompt Aggregation (UPA) model for final inference...")
    ...
    def get_uniform(self, epochs):
        uniform = lambda x: 1 / epochs
        return uniform
